...
/** Punctuation allowed between fodder words for smooth grammar (not decoration). */
...
export const SURFACE_MISDIRECTION_RULE =
  "Quotation marks, parentheses, asterisk italics (*like this*), and similar devices may dress the whole clue surface — a reported remark, a bracketed aside around the full sentence, a rhetorical frame — to misdirect the reader, not to highlight fodder. Never bracket or quote only the fodder cluster or individual fodder words; decoration must not outline wordplay. The surface must still read as smooth, grammatical English.";
...
2. Fodder words (${anagramFodder.split(/\s+/).join(", ")}) must ALL appear in the clue with exact spelling (capitalise proper names). You may reorder them. ${FODDER_PUNCTUATION_RULE}
3. ${SURFACE_MISDIRECTION_RULE}
4. Anagram indicator sits with the fodder/wordplay half (before or after the fodder cluster is fine).
5. Use at most ${MAX_LINKING_WORDS} linking words between definition and wordplay — only words that genuinely connect the two halves; ${MAX_LINKING_WORDS} is the maximum, fewer is better. Nothing superfluous: no filler, no padding, no redundant phrasing, and no extra words in the wordplay half beyond fodder and indicator.
6. End with ${enumeration}.
7. Do not write any word from ${answer} as a standalone word in the clue.
