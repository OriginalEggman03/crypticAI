...
const outputs: { path: string; width: number; height: number }[] = [
  { path: join(root, "app", "icon.png"), width: 512, height: 512 },
  { path: join(root, "app", "apple-icon.png"), width: 512, height: 512 },
...
async function main(): Promise<void> {
  ...
  for (const out of outputs) {
    const svg = renderBrandSvg({ width: out.width, height: out.height });
